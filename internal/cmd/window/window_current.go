...
// newCurrentCommand creates the window current command.
func newCurrentCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "current",
		Short: "Show the current window ID",
		Long:  "Display the ID of the current iTerm2 window based on the current session",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, session := range sessions {
				if session.SessionID == sessionID {
					// Output just the window ID for easy scripting
					fmt.Println(session.WindowID)
					return nil
				}
			}
