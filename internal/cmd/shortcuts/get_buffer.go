...
// newGetBufferCommandImpl creates the implementation of the top-level get-buffer command.
func newGetBufferCommandImpl() *cobra.Command {
	template := cmdutil.CommandTemplate{
		Use:   "get-buffer [<session-id>]",
		Short: "Get buffer contents with scrollback (shortcut for session get-buffer)",
		Long: `Get buffer contents of a session including scrollback history.

This is a convenience wrapper for 'it2 session get-buffer'.`,
		Example: cmdutil.Doc(`
			# Get current session buffer
			$ it2 get-buffer

			# Get specific session buffer
			$ it2 get-buffer SESSION123

			# Get last 100 lines only
			$ it2 get-buffer --lines 100
			$ it2 get-buffer --last 20

			# Include color information
			$ it2 get-buffer --color

			# Save session output to file
			$ it2 get-buffer > session-output.txt
		`),
