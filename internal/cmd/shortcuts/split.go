...
// newSplitCommandImpl creates the implementation of the top-level split command.
func newSplitCommandImpl() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "split [<session-id>]",
		Short: "Split a session pane (shortcut for session split)",
