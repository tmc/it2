...
// ContextualTabIDCompletion provides tab completion filtered by window context
func ContextualTabIDCompletion(windowID string) func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	return func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		tabIDs := getTabIDsForWindow(cmd, windowID)
		var completions []string
		for _, tabID := range tabIDs {
			if strings.HasPrefix(tabID, toComplete) {
				completions = append(completions, tabID)
			}
		}
		return completions, cobra.ShellCompDirectiveNoFileComp
	}
...
// ContextualSessionIDCompletion provides session completion filtered by window/tab context
func ContextualSessionIDCompletion(windowID, tabID string) func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	return func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		wsURL := cmd.Root().PersistentFlags().Lookup("url").Value.String()
		timeout, _ := time.ParseDuration(cmd.Root().PersistentFlags().Lookup("timeout").Value.String())
		...
		// Resolve window ID if needed
		resolvedWindowID := resolveWindowID(sessions, windowID)

		var completions []string
		for _, session := range sessions {
			// Filter by window
			if windowID != "" && session.WindowID != resolvedWindowID {
				continue
			}
			// Filter by tab
			if tabID != "" && session.TabID != tabID {
				continue
			}
