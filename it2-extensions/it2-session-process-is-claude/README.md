...
Process enricher plugin that detects if a process is running Claude Code.
...
The plugin outputs "true" if the process name contains "claude", indicating it's running Claude Code.

## Example Usage

```bash
# Call the plugin directly
./it2-session-process-is-claude.sh "ABC123" "12345"
# Output: true (if process 12345 is claude)

# Plugins are automatically discovered when in PATH
# They enrich process-list output:
it2 session process list
```

## Output

When a process is identified as Claude Code, the plugin outputs:
- `true` - Process is running Claude Code
- (empty) - Process is not Claude Code
